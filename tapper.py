        ...
        try:
            ...
            if response.status == 400:
                ...
                if response_json["error_code"] == "UPGRADE_COOLDOWN":
                    ...
                    logger.info(f"{self.session_name: <8} | 🆙 <e>{upgrade_id}</e> | ⏳ <y>{round(self.upgrade_cooldowns[upgrade_id] - time())}</y> сек.")
                    return False
            if response.status != 422:
                response.raise_for_status()
                ...
                try:
                    if time() - access_token_created_time >= 3600:
                        ...
                        if not exchange_id:
                            ...
                            if status is True:
                                logger.success(f"{self.session_name: <8} | 💰<e>{total}</e> | <y>{earn_on_hour}</y><g>/ч</g> | 💵<c>{balance}</c> | Успешно выбрана биржа <y>Bybit</y>")

                        last_passive_earn = profile_data['lastPassiveEarn']
                        earn_on_hour = profile_data['earnPassivePerHour']

                        logger.info(f"{self.session_name: <8} | Последний пассивный доход: <g>+💵 {last_passive_earn}</g> | "
                        ...
                        if is_completed is False:
                            ...
                            if status is True:
                                logger.success(f"{self.session_name: <8} | 💰<e>{total}</e> | <y>{earn_on_hour}</r><g>/ч</r> | 💵<c>{balance}</c> | Успешно получена дневная награда! | "
                    ...
                    if active_turbo is False:
                        if (settings.APPLY_DAILY_ENERGY is True
                            ...
                            logger.info(f"{self.session_name: <8} | 💰<e>{total}</e> | <y>{earn_on_hour}</y><r>/ч</r> | 💵<c>{balance}</c> | ⏳ <y>5</y> сек. | активация ⚡")
                            await asyncio.sleep(delay=5)

                            status = await self.apply_boost(http_client=http_client, boost_id="BoostFullAvailableTaps")
                            if status is True:
                                logger.success(f"{self.session_name: <8} | 💰<e>{total}</e> | <y>{earn_on_hour}</y><r>/ч</r> | 💵 <c>{balance}</c> | Активация ⚡")
                        ...
                        if settings.AUTO_UPGRADE is True:
                            ...
                            for upgrade in available_upgrades:
                                ...
                                price = 1 if price == 0 else price
                                profit = 1000 if profit == 0 else profit
                            ...
                            logger.info(f"{self.session_name: <8} | 💰<e>{total}</e> | <y>{earn_on_hour}</y><r>/ч</r> | 💵<c>{balance}</c> | 🆙 <e>{best_upgrade[0]}</e> <m>{best_upgrade[2]}</m> lvl | 💵 <y>{best_upgrade[3]}</y> | ⏳ <y>{round(time_to_be_earned * 60 * 60 if time_to_be_earned > 0 else 0)}</y> сек.")
                            if balance >= best_upgrade[3]:
                        

                                if balance > best_upgrade[3] and best_upgrade[2] <= settings.MAX_LEVEL:
                                    logger.info(f"{self.session_name: <8} | 💰<e>{total}</e> | <y>{earn_on_hour}</y><r>/ч</r> | 💵<c>{balance}</c> | ⏳ <y>{TIME_TO_WAIT_BEFORE_UPGRADE}</y> сек. | 🆙 <e>{best_upgrade[0]}</e>")
                                    ...
                                    if status is True:
                                        ...
                                        logger.success(
                                            f"{self.session_name: <8} | 💰<e>{total}</e> | <y>{earn_on_hour}</y>(<g>+{best_upgrade[4]}</g>)<r>/ч</r> | 💵 <c>{balance}</c> | "
                                            f"🆙 <e>{best_upgrade[0]}</e> <m>{best_upgrade[2]}</m> lvl")
                                        await asyncio.sleep(delay=1)

                                continue

                        if available_energy <= settings.MIN_AVAILABLE_ENERGY:
                            logger.info(f"{self.session_name: <8} | 💰<e>{total}</e> | <y>{earn_on_hour}</y><r>/ч</r> | 💵<c>{balance}</c> | Прокликал до ⚡<m>{available_energy}</m>")
                            logger.info(f"{self.session_name: <8} | 💰<e>{total}</e> | <y>{earn_on_hour}</y><r>/ч</r> | 💵<c>{balance}</c> | ⏳ <y>{settings.SLEEP_BY_MIN_ENERGY}</y> сек.")
                    ...
                    logger.info(f"{self.session_name: <8} | 💰<e>{total}</e> | <y>{earn_on_hour}</y><r>/ч</r> | 💵<c>{balance}</c> | ⏳ <y>{sleep_between_clicks}</y> сек.")
